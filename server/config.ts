...
const envSchema = z.object({
  ...
  JWT_SECRET: z.string().optional().transform(v => (v && v.trim() !== '' ? v : 'interviewlab-default-jwt-secret-key-2026-safe-fallback')).default('interviewlab-default-jwt-secret-key-2026-safe-fallback'),
  OTEL_EXPORTER_OTLP_ENDPOINT: z.string().optional().transform(v => (v && v.trim() !== '' ? v : 'http://localhost:4318')).default('http://localhost:4318'),
  OTEL_SERVICE_NAME: z.string().optional().transform(v => (v && v.trim() !== '' ? v : 'interviewlab-api')).default('interviewlab-api'),
